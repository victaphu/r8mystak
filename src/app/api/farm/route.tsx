...
async function main(owner: string, postId: string) {
  ...
  // Consumer contract
  const consumerAddress = "0xF0f6C2D960965dAb65E3e12EE3ED8eBf0224F460";
  const consumerAbiPath =
    "./build/artifacts/contracts/STAKController.sol/STAKController.json";
  ...
  const oracleAbiPath =
    "./build/artifacts/contracts/dev/functions/FunctionsOracle.sol/FunctionsOracle.json";
  ...
  let doGistCleanup: any;
  let gistUrl: any;
  if (typeof secrets !== "undefined") {
    const result: any = await getEncryptedSecrets(secrets, oracle, signerPrivateKey);
    if (isObject(secrets)) {
  ...
  await new Promise(async (resolve, reject) => {
    ...
    const requestEvent = requestTxReceipt.events.filter(
      (event: any) => event.event === "RequestSent"
    )[0];
...
async function getEncryptedSecrets(secrets: any, oracle: any, signerPrivateKey: any = null) {
...
// in the starter kit.
async function verifyOffchainSecrets(secretsURLs: any, oracle: any) {
...
// Code is from ./FunctionsSandboxLibrary/encryptSecrets.js
async function encrypt(readerPublicKey: any, message: any) {
...
const createGist = async (githubApiToken: any, encryptedOffchainSecrets: any) => {
...
// code from ./tasks/utils
const checkTokenGistScope = async (githubApiToken: any) => {
  const headers = {
...
// code from ./tasks/utils
const deleteGist = async (githubApiToken: any, gistURL: any) => {
  const headers = {
    Authorization: `Bearer ${githubApiToken}`,
  };
...
// main('0xD1dB220F3664c741f195bd252E3e33f4Ad954404', '0x8567-0x01-DA-cc7c1901')
//   .then(() => process.exit(0))
//   .catch((error) => {
//     console.error(error);
//     process.exit(1);
...
export async function POST(request: Request) {
  ...
  const { address, postId } = req.body;

  await main(address, postId);
  ...
  var provider = ethers.providers.getDefaultProvider('mumbai');

  var _address = "0x15C8632A67F66Dd0c764E56760C98A1dA3f19641";
  var contract = new ethers.Contract(_address, abi, provider);

  return NextResponse.json({
    instaLikes: 0,
    tiktokLikes: 0,
    postLikes: await contract.balanceOf(address)
  });
}