...
const staks = require("./abis/STAKController.json");
const functionOracle = require("./abis/FunctionsOracle.json");

async function main(owner, postId) {
  ...
  // Default example
  const source = await fs.readFile("./src/app/api/farm/abis/function.txt", "utf8");
  const args = [postId];
  ...
  const oracleAbi = functionOracle.abi;
...
export default async function handle(address, post) {
    return await main(address, post);
}
